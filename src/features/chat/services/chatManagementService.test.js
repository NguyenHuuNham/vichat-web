...
test('group owner departure transfers to survivors but lets the final member close the group', () => {
  ...
  assert.match(appSource, /function groupActiveMembers[\s\S]*roomParticipantIds\(room\)/);
  assert.match(appSource, /if \(room\.managementSnapshot\) return account/);
  assert.match(appSource, /function groupOwnerReplacementMembers[\s\S]*groupActiveMembers\(room, accounts\)/);
  assert.match(appSource, /const closesEmptyGroup = groupActiveMembers\(targetRoom, directoryAccounts\)[\s\S]*\.every\(member => identitiesOverlap\(member, currentUser\)\)/);
  ...
  assert.match(managementServiceSource, /async deleteConversationForCurrentUser\(conversationId, \{ replacementId = '' \} = \{\}\)/);
  assert.match(managementServiceSource, /\/conversation\/\$\{encodeURIComponent\(conversationId\)\}\/self/);
  assert.match(appSource, /deleteConversationForCurrentUser\([\s\S]*\{ replacementId \}/);
  assert.match(appSource, /if \(activeChat\.isGroup\) \{\s*requestGroupLeave\(activeChat, 'delete'\)/);
