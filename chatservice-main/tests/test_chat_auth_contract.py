...
class ChatAuthContractTests(unittest.TestCase):
    ...
    def test_group_access_repair_is_additive_and_does_not_mutate_chatmgt(self):
        ...
        self.assertIn("account_realtime_eligible", repair_source)
        self.assertIn('groups_skipped_legacy_only', repair_source)
        self.assertNotIn("tinode_sso_login", repair_source)
    ...
    def test_group_member_approval_stays_out_of_active_tinode_membership_until_approved(self):
        ...
        self.assertIn("_is_group_manager(viewer_membership)", serializer_source)
        self.assertNotIn('_is_admin({"role": getattr(viewer_account, "role", "")})', serializer_source)
        ...
        self.assertIn("if not _is_group_manager(membership)", approval_source)
        self.assertNotIn("_is_admin(current_user)", approval_source)
    ...
    def test_group_creation_viewer_and_avatar_updates_preserve_realtime_contract(self):
        ...
        self.assertIn("onPointerDown={handlePointerDown}", app_source)
        self.assertIn("setImageViewer(null);", app_source)
        self.assertIn("setShareMessage(shareableMessage);", app_source)
        self.assertIn("tinodeClient.fetchFile(sourceAttachment)", app_source)
        self.assertNotIn("navigator.share", app_source)
        ...
        self.assertIn("if (metadata.sharedFrom) draft.head['x-shared-from']", tinode_source)
    ...
    def test_group_settings_are_owner_only_and_tenant_scoped(self):
        ...
        self.assertIn("Bạn chưa được admin cấp phép.", controller_source)
    ...
    def test_directory_presence_uses_ephemeral_tenant_scoped_leases(self):
        ...
        self.assertIn("_LAST_SEEN_MONOTONIC_SCRIPT", presence_source)
        self.assertIn("evaluator(", presence_source)
