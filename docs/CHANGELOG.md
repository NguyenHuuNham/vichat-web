...
- Thoi gian: 2026-08-11 10:33 (Asia/Saigon)
- Loai: Tinh nang | Bao mat | API | Ha tang | Kiem thu | Tai lieu
- Trang thai: Da commit/push; dang deploy co kiem soat sau mot lan rollback khoi dong
...
- Kiem thu: `npm run test:frontend` dat 55/55; `npm run lint` exit 0 (chi warning legacy/vendor/worktree co san); `npm run build:production` dat voi bundle `App-GtSLfGNI.js`; `python -m unittest discover -s chatservice-main/tests -q` dat 127 test, skip 39 test can dependency runtime; `python -m py_compile ...` dat; `docker compose -f infrastructure/production/compose.yaml config --no-interpolate -q` dat; `git diff --check` dat. Build/image production phat hien va sua hai loi entrypoint truoc UAT: `asyncio.Lock()` tao luc import khong tuong thich `uvloop`, sau do worker `python scripts/...` thieu `/app` trong `sys.path`; ca hai deu co regression test, Chatmgt moi da healthy HTTP 200 va worker duoc rebuild rieng.
- Viec tiep theo: Dat `TINODE_CHATBOT_PASSWORD` va `TINODE_CHATBOT_WEBHOOK_KEY` moi tren production, recreate chi `chatmgt`, `tinode-chatbot-webhook` va `chat` neu can; xac minh worker health, bot UID, tin bot, sau do doi chieu chat nhan vien/group/file/upload van binh thuong.
