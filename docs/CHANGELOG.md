...
- Thoi gian: 2026-09-02 16:29-17:23 (Asia/Saigon)
- Loai: Tinh nang | Bao mat | Web | Mobile | Backend | Ha tang | Du lieu | Kiem thu | Tai lieu
- Trang thai: Hoan tat code va kiem tra local; chua commit, chua cau hinh/deploy production
...
- Kiem tra production: Archive `/opt/deploy/chat/incoming/vichat-conversation-categories-bda3e1f.tar.gz` 44857665 bytes co SHA-256 `2cee7d8dfe30eb4e92f1aad2edb365db3430460617277798fbc5a4b5bd8373fa` khop local/remote; Compose config, build rieng image `chat` va `nginx -t` preflight deu dat. ChatUI `33e0ad01125d` healthy voi image `sha256:5e17a2a6ee9d6007bd383ea0869000edce09cad5e2556930038e02ecba4dcd8e`; local `http://127.0.0.1:8094/healthz`, public `https://chat.upgo.vn/healthz` va Chatmgt auth health deu tra OK. Public `/assets/index-CH5sy2cH.js`, `/assets/App-BexGFA9t.js` va `/assets/index-8FEik51U.css` khop SHA-256 tung byte voi container; App co marker `Quan ly the phan loai` va `vichat.conversation-categories.v2`, CSS co marker modal/color picker. `sudo -n nginx -t` dat; log ChatUI/Chatmgt/bridge/webhook 10 phut co 0 marker fatal/panic/traceback/uncaught/critical/emerg.
...
- Kiem thu: Test dich web/demo dat 43/43; `npm run test:frontend` dat 243/243; Chatmgt auth contract dat 48/48; full backend `python -m unittest discover -s chatservice-main/tests -q` dat 193 test, skip 67 do dependency runtime; `python -m py_compile` dat; `npm run lint` exit 0 voi warning legacy/vendor da co; mobile `typecheck` va ESLint dat. `npm --prefix mobile run test` co 29 test dat nhung mot suite `workspaceService.test.ts` khong khoi dong do Vitest/Rolldown khong parse Flow trong `react-native/index.js`, khong di qua code thay doi nay. `npm run build:production` dat voi canh bao chunk App lon hon 500 KB da co; `git diff --check` dat. Image production chay lai `py_compile`, auth contract 48/48 va `nginx -t` tren Compose network deu dat.
- Kiem tra production: Archive `/opt/deploy/chat/incoming/vichat-final-owner-leave-17214c2.tar.gz` SHA-256 `1387efefbe880ad20bed51ac69b4feab7667d840777aac429a2e4a5979cfecae`; backup Chatmgt PostgreSQL `/opt/deploy/chat/backups/final-owner-leave-17214c2-20260825-1000/chatservice.dump` 140399 bytes, mode `0600`, SHA-256 `067d993ee461c88b749f724ec1df0f1b10e27d480e23e35c725a6e6032eef7d6`, da kiem tra duoc bang PostgreSQL 16. Chatmgt `e246a8dc1eea` healthy voi image `sha256:34c9bf2853b0596e919a734fad5587d11d346d8a72ba68424ab2bdcd3f420e84` va bind mount truc tiep release moi; ChatUI `23cfe32fcc5b` healthy voi image `sha256:ed57f4caef37b1276a22551d84e01314e7f2db4ed16a4a99ab26d0e29333d907`. Local/public ChatUI `/healthz` va Chatmgt auth health tra OK; public bundle `/assets/index-Bq2tp_Rc.js`, `/assets/App-C5jZghdC.js`, `/assets/index-CISnHTLs.css` khop byte voi container va co marker sole-owner leave; Alembic van `20260824_12 (head)`; `sudo -n nginx -t` dat; log ChatUI/Chatmgt/bridge/webhook 15 phut co 0 marker fatal/panic/traceback/uncaught/critical/emerg.
...
- Kiem tra production: Archive `/opt/deploy/chat/incoming/vichat-group-info-permissions-c2742fc.tar.gz` SHA-256 `a996ffa421f17c52b419d25109308998694c409f63cf8d34e1cd41eb2dd77cc3`; backup Chatmgt PostgreSQL `/opt/deploy/chat/backups/group-info-c2742fc-20260825-0912/chatservice.dump` 140531 bytes, mode `0600`, SHA-256 `84013f41e5f9a07b0cd8250440411bdef77ac025e0bc4272f2d15107f342c6b1`. Compose config, production image build, `py_compile`, Nginx trong Compose network va image contract dat 48/48; hai lan contract image dau dung thieu/sai read-only source mount nen chi loi `FileNotFoundError /src/app/App.jsx`, khong recreate service, lan chay voi cac mount `src`/`infrastructure`/`docs`/`scripts` dung dat. Mot lan `docker run nginx -t` doc lap khong co Compose DNS nen khong resolve `chatmgt`; chay lai trong Compose network dat. ChatUI `c2f340bf69a0` healthy voi image `sha256:d3016e69647b30466fa34edcf9070783590e12b0f7e51d8fdc143af7d572920a`; Chatmgt `20d1ede32cfb` healthy voi image `sha256:5568713783dbe60b5d6306d0af611bf3ec720896719a5bdb22a8b27d63a22834` va bind mount truc tiep vao release bat bien. Local/public ChatUI `/healthz` va Chatmgt auth health tra OK; public entry `/assets/index-ndEiDooD.js`, App `/assets/App-D3fQU-Uo.js`, CSS `/assets/index-CISnHTLs.css` HTTP 200 va co marker permission/realtime/activity/locked background; Alembic van o `20260824_12 (head)`; `sudo -n nginx -t` dat; log ChatUI/Chatmgt/bridge/webhook 15 phut co 0 marker fatal/panic/traceback/uncaught/critical/emerg.
- Trien khai: Source commit `c2742fc` da push `origin/master`; release `/opt/deploy/chat/releases/group-info-permissions-c2742fc-20260825-0915` dang la `current`, `previous` tro `/opt/deploy/chat/releases/conversation-sync-a321772-20260825-0738`. `chat` duoc recreate mot lan; `chatmgt` duoc recreate lan cuoi tu duong dan release tuyet doi sau khi lan dau ghi bind source qua symlink `current`, de mount production tro truc tiep release bat bien. Ca hai dung `--no-deps --force-recreate --no-build`; khong migration, khong `down`, khong reset database/volume/topic/message/read cursor/avatar. Rollback tags `songhong-production-chat:rollback-before-group-info-c2742fc` va `songhong-production-chatmgt:rollback-before-group-info-c2742fc` giu image cu. Chat PostgreSQL `78a434b49404`, ChatAPI `8476615ad4ac`, Coturn `aa680d35fdc0`, Redis `ceef7df23feb`, account bridge `7d9c0e317719`, webhook `2882b6109176` va Tinode PostgreSQL `9f6e4dcc9c2f` giu nguyen container ID.
...
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`; UAT bang owner/member tren hai session: thu hoi/cap lai `allowMembersEditInfo`, kiem tra nut ten/avatar an-hien realtime, thu shared background khi bi khoa, va xac nhan activity ten/avatar/quyen/hinh nen hien trong ca hai timeline ma khong tao notification cho tin cu.
- Commit/PR: Source commit `c2742fc`; deployment follow-up docs commit `e09734e`; khong co PR.
...
- Kiem tra production: Archive `/opt/deploy/chat/incoming/vichat-tenant-tight-b5bd160.tar.gz` SHA-256 `c40a2610b63ab57e06cb5ee5d01ef4813aa01644a3f1152ba8808b18301733d0`; Compose preflight va image preflight dat; container ChatUI `ab17528bdb6a` healthy voi image `sha256:1b954694a7c407366a390a06f11075bac268d57ae09d250fe7037ff0ccdf94dc`; local/public `/healthz` va Chatmgt auth health tra OK; public entry `/assets/index-dl32OAfk.js`, App `/assets/App-B1ZmBXu4.js` va CSS `/assets/index-B3autpXX.css` HTTP 200, co marker `tenant-switcher-toggle`, `tenant-switcher-menu`, `tenant-switcher-current-name`, `width:min(128px,18vw)` va responsive `width:min(128px,100vw - 154px)`; `sudo -n nginx -t` dat; log ChatUI/Chatmgt 10 phut khong co marker fatal/panic/traceback/uncaught/critical/emerg.
- Trien khai: Source commit `b5bd160` da push `origin/master`; release `/opt/deploy/chat/releases/tenant-tight-b5bd160-20260825-1340` dang la `current`, `previous` tro `/opt/deploy/chat/releases/tenant-arrow-bf9e67f-20260825-1309`; chi recreate `chat` voi `--no-deps --force-recreate --no-build`, khong migration/reset database, Tinode, avatar, read cursor hay volume. Rollback tag `songhong-production-chat:rollback-before-tenant-tight-b5bd160` giu image cu `sha256:9a816e66c68c28fb60962f1ecd4f69389091100410f29b985b780a9889670502`; Chatmgt `09ce43ebc155`, webhook `2882b6109176`, account bridge `7d9c0e317719`, ChatAPI `8476615ad4ac`, Coturn `aa680d35fdc0`, Chat PostgreSQL `78a434b49404`, Tinode PostgreSQL `9f6e4dcc9c2f` va Redis `ceef7df23feb` giu nguyen container ID.
- Rui ro con lai: Chua UAT pixel-level bang browser trong phien nay vi cong cu browser khong duoc expose; can hard refresh va kiem tra desktop/mobile voi tai khoan co nhieu cong ty.
...
- Commit/PR: Source commit `b5bd160`; deployment follow-up docs commit duoc tao sau khi cap nhat muc nay.
...
- Loai: Sua loi | Bao mat | Web | Realtime | Kiem thu | Van hanh | Tai lieu
- Trang thai: Da commit va deploy production; san sang UAT
...
- Kiem thu: `node --test src/features/chat/services/chatRealtime.test.js src/features/chat/services/chatManagementService.test.js src/features/chat/services/unreadBoundary.test.js` dat 72/72; `npm run test:frontend` dat 234/234; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat, canh bao chunk App lon hon 500 KB da co; `python -m unittest discover -s chatservice-main/tests -p "test_chat_auth_contract.py" -q` dat 48/48; `python -m unittest discover -s chatservice-main/tests -q` hoan tat 193 test, skip 67 do dependency runtime; `python -m py_compile chatservice-main/application/controllers/api_chat_management.py` dat; `git diff --check` dat. Image Chatmgt production chay lai auth contract voi source release mount read-only dat 48/48 va `py_compile` dat.
- Kiem tra production: Archive `/opt/deploy/chat/incoming/vichat-avatar-read-28e0c96-20260825.tar.gz` SHA-256 `b0f9e39344ba5bac403fa41c3c40bb82101506ea189330be68c482f1296ce3fb`; backup Chatmgt PostgreSQL `/opt/deploy/chat/backups/avatar-read-28e0c96-20260825-0150/chatservice.dump` 140162 bytes, SHA-256 `e6f1fdf5da3f7873f86c342be02c42a5c71ec2cfd39679412f30403167f70c97`, mode `0600`. Release `/opt/deploy/chat/releases/avatar-read-28e0c96-20260825-0150` dang la `current`, `previous` tro `/opt/deploy/chat/releases/tenant-loading-058dcf0-20260825-0026`; ChatUI image `sha256:f0841946ee67ccb2cc7c1252be0760e9b9e932aa64cbc898c4b670328a874e0c`, container `54a185b6c526` healthy; Chatmgt image `sha256:dcc12814a6344230c57ade3d6f1842bab58d9a0b686c76efacb73999aa14c0e2`, container `09ce43ebc155` healthy. Local/public `/healthz` tra `ok`; Chatmgt public health tra `status=ok`, management data va Tinode bridge configured; bundle `/assets/index-BM9UmCPf.js`, `/assets/App-CtKogQ6n.js` va `/assets/index-8tRmmT_P.css` HTTP 200; `sudo -n nginx -t` dat; log ChatUI/Chatmgt 20 phut co 0 marker `fatal|panic|traceback|uncaught|critical|emerg`.
- Trien khai: Chi recreate `chatmgt` roi `chat` voi `--no-deps --force-recreate --no-build`; khong migration, khong `down`, khong reset topic/message/volume. Tinode bridge `7d9c0e317719`, webhook `2882b6109176`, ChatAPI `8476615ad4ac`, Redis `ceef7df23feb`, PostgreSQL `78a434b49404`/`9f6e4dcc9c2f` va Coturn `aa680d35fdc0` giu nguyen ID. Hai preflight staging truoc dung o contract test do thieu read-only source mount va khong recreate service; release active chi duoc chuyen sau khi gate da dat.
- Rui ro con lai: Chua UAT production bang hai tai khoan Tinode that cho group avatar, read cursor va notification; can hard refresh de nap bundle moi.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, dung hai tai khoan mo lai chat da doc tu hom truoc va nhom co avatar; xac nhan khong hien unread/thong bao cu, avatar khong mat va tin nhan van con nguyen.
- Commit/PR: Source commit `28e0c96` da push `origin/master`; deployment follow-up docs commit duoc tao sau khi cap nhat muc nay.
...
- Noi dung: Bo sung loc fail-closed chi chap nhan account co `tenantId` trung tenant dang active; response `listUsers/searchUsers`, khoi tao danh ba, dong bo nen, ket qua tim kiem, nickname va danh sach ID presence deu duoc loc lai truoc khi vao state. Loai bo fallback tenant build-time khi chuan hoa account de snapshot thieu tenant khong bi gan nham.
...
- Noi dung: Hien cong ty hien tai trong box compact gom logo, ten va trang thai; nut mui ten xuong mo menu rieng. Menu hien logo, ten, trang thai cua tung cong ty theo mot cot va gioi han chieu cao de cuon doc; click cong ty khac van goi luong xac nhan truoc khi switch. Bo sung mau dark theme rieng cho nut mui ten va noi dung tenant de giu tuong phan.
- Quyet dinh ky thuat: Giu `requestTenantSwitch` va co che dong menu khi click ngoai/Escape; chi thay lop hien thi, khong tao endpoint hay state tenant dai han moi.
- Database/API/cau hinh: Khong migration, endpoint, schema, secret hoac bien moi truong moi.
...
## 2026-08-24-30 - Rut gon bo chuyen cong ty trong ho so
...
- Noi dung: Bo carousel mui ten trai/phai; hien nhan `Cong ty`, logo tenant hien tai va mui ten trong mot control compact. Khi mo mui ten, cac tenant hien thanh icon logo; hover/focus co tooltip ten cong ty, icon hien tai co dau check. Chon tenant khac van di qua modal xac nhan va luong switch/reload hien co.
- Quyet dinh ky thuat: Dung menu icon trong ChatUI va title/data tooltip tu ten tenant da duoc Chatmgt normalize; khong tao state dai han, endpoint moi hay ghi them du lieu tenant.
- Database/API/cau hinh: Khong migration, endpoint, schema, secret hoac bien moi truong moi.
...
- Noi dung: Tinode subscriber metadata `read`/`recv` duoc chuyen thanh danh sach receipt theo tung tin outgoing; receipt event realtime van cap nhat status cu va bo sung UID nguoi xem. UI uu tien nguoi `Da xem`, hien toi da 5 avatar va nhan `5+`; click vao avatar/check mo preview tin nhan, thong tin nguoi gui/thoi gian va hai nhom nguoi `Da xem`/`Da nhan`.
- Quyet dinh ky thuat: Khong tao API hoac state local dai han moi; Tinode van la source of truth. Helper rieng chuan hoa UID, loai trung, loai nguoi da xem khoi danh sach da nhan va gioi han preview de tranh lam roi luong receipt/realtime hien co.
- Database/API/cau hinh: Khong migration, endpoint, schema, secret hoac bien moi truong moi.
...
- Noi dung: Mention picker van hien nhan viewer-specific nhung chen token chinh thuc theo `defaultName`; `x-mentions` chi gui cac truong allow-list va ten chinh thuc. Reply metadata gui kem sender ID/ten chinh thuc, target cua event them/xoa thanh vien va ten nguoi gui khi forward cung duoc chuan hoa. Khi render, ChatUI resolve stable ID ve danh ba cua viewer de hien lai biet danh rieng; legacy mention co ID hop le cung duoc hien theo viewer hien tai.
- Quyet dinh ky thuat: Giu noi dung Tinode dung chung va on dinh; khong ghi nickname vao transport. Chi lop render/composer moi dung ten viewer-scoped, tranh thay doi luong Chatmgt, realtime, receipt, message history va database.
- Database/API/cau hinh: Khong migration, endpoint, schema, secret hoac bien moi truong moi.
...
- Database/API/cau hinh: Khong migration, endpoint, schema, secret, bien moi truong hoac thay doi kien truc.
- Kiem thu local: `npm run test:frontend` dat 208/208; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build` va `npm run build:production` dat voi canh bao chunk App lon hon 500 KB; `node --test src/features/chat/services/stickerCatalog.test.js src/features/chat/services/conversationNotifications.test.js src/features/chat/services/keyboardShortcuts.test.js` dat 16/16; `git diff --check` dat.
- Kiem thu production: Release `docker compose config --quiet` dat; image ChatUI `sha256:f34c2ef0325410ab3887161363c200c0578e6492f62928c8e5452c5c458af0d4` co marker `stickerSuggestions`, `keyboardShortcuts`, `sticker-suggestion`; `chat` healthy; local `http://127.0.0.1:8094/healthz` va public `https://chat.upgo.vn/healthz` tra `ok`; `https://chatmgt.upgo.vn/api/v1/auth/health` tra HTTP 200/status `ok`; `sudo -n nginx -t` dat; public bundle `App-x4dkRVHc.js` co du marker; log ChatUI 10 phut sau recreate khong co fatal/panic/traceback/uncaught/critical/emerg/error.
- Trien khai: Source commit `8beeb6b` da push `origin/master`; archive `/opt/deploy/chat/incoming/vichat-shortcuts-stickers-8beeb6b.tar.gz` SHA-256 `c7fb572e9132d916fa60d86a793715172adc30b8dd6e83006365d34239114307`; release `/opt/deploy/chat/releases/shortcuts-stickers-8beeb6b-20260824-1648` dang la `current`, `previous` tro `/opt/deploy/chat/releases/direct-chat-deletion-c171785-20260824-090115`; chi recreate `chat`, khong migration va khong restart Chatmgt, Tinode, bridge, webhook, ChatAPI, PostgreSQL, Redis, Coturn. Container ChatUI moi `b7453b0e1baa`; Chatmgt `5aa69e4f4129`, Tinode bridge `7d9c0e317719`, chatbot webhook `2882b6109176`, ChatAPI `8476615ad4ac`, Redis `ceef7df23feb`, PostgreSQL `78a434b49404`/`9f6e4dcc9c2f`, Coturn `aa680d35fdc0` giu nguyen ID.
- Rui ro con lai: Chua UAT pixel/browser bang tai khoan that trong phien nay; can hard refresh va kiem tra card Phim tat, bat/tat/custom phim, goi y `deadline`/`cam on`, click gui sticker o chat 1-1 va group, toggle tat goi y, cung tai khoan cu da co PIN.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn` va UAT hai luong tren; neu loi chi tro `current` ve `previous` va recreate rieng `chat`, khong reset Tinode/topic/message hay database.
- Commit/PR: Source commit `8beeb6b` da push `origin/master`; deployment record follow-up changelog commit.
...
- Kiem thu: `npm run test:frontend` dat 201/201 sau deploy; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build` va `npm run build:production` dat voi canh bao chunk App lon hon 500 KB da co; `python -m unittest discover -s chatservice-main/tests -q` dat 186 tests, skip 63; targeted presence/auth dat 47 tests, skip 2; `py_compile`, `git diff --check` va Compose config voi gia tri validation khong bi mat deu dat.
- Rui ro con lai: Chua UAT production bang hai tai khoan that; offline khi mat mang/phat crash phu thuoc TTL toi da 8 giay neu pagehide khong gui duoc.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn` va UAT hai tai khoan: online len dau trong khoang 2 giay, offline sau logout/pagehide hoac TTL toi da 8 giay, khong thay presence khac tenant. Neu loi chi tro `current` ve `previous` va recreate rieng `chatmgt`/`chat`; khong reset Tinode/topic/message hay database.
- Trien khai: Source commit `d010d89` da push `origin/master`; archive `/opt/deploy/chat/incoming/vichat-directory-presence-d010d89.tar.gz` SHA-256 `6c010b1b2d36c59097fc8b505e71fb0a549459c76946c63f21d6d4bfbd1c3a4e`; release `/opt/deploy/chat/releases/directory-presence-d010d89` dang la `current`, `previous` tro `/opt/deploy/chat/releases/directory-presence-c9dbc9e-20260824-1233`. Chi recreate `chatmgt` va `chat` voi `--no-build --no-deps --force-recreate`; khong migration, khong `down`, khong restart Tinode, bridge, webhook, ChatAPI, PostgreSQL, Redis, Coturn hay volume. Image ChatUI `sha256:f961f80de57273f938a110421d0e0628b0dcec50c242ac03c0aa3b91ffcd5f78`, container `5cac4aad02c2`; image Chatmgt `sha256:579e84b93a6b5929228a2a0891e2af42d2f3b215cfd71dd274bceef0ef3c898f`, container `fb1b275efdaa`; rollback tags `songhong-production-chat:rollback-before-directory-presence-c9dbc9e-1233` va `songhong-production-chatmgt:rollback-before-directory-sync-814fc0b` da co san.
- Kiem tra production: `http://127.0.0.1:8094/healthz`, `https://chat.upgo.vn/healthz` va `https://chatmgt.upgo.vn/api/v1/auth/health` tra HTTP 200; hai container healthy; `sudo -n nginx -t` dat; public bundle `App-DCXcvVh2.js` HTTP 200 va co marker `heartbeatPresence`, `presence/batch`, `presence/offline`; log 15 phut cua ChatUI/Chatmgt khong co fatal/panic/traceback/critical/emerg/uncaught. Container Tinode webhook `2882b6109176`, account bridge `7d9c0e317719`, ChatAPI `8476615ad4ac`, Redis `ceef7df23feb`, hai PostgreSQL `78a434b49404`/`9f6e4dcc9c2f` va Coturn `aa680d35fdc0` giu nguyen ID.
- Commit/PR: Source `d010d89`; deployment record `59d8118` da push `origin/master`.
...
- Commit/PR: Source `c9dbc9e` va deployment record `b1dddc0` da push `origin/master`.
...
- Kiem thu: `python -m unittest discover -s chatservice-main/tests -q` dat 183 tests, skip 61; `python -m unittest chatservice-main/tests/test_chat_auth_contract.py -q` dat 44/44; `python -m py_compile chatservice-main/application/controllers/api_chat_management.py chatservice-main/tests/test_chat_auth_contract.py` dat; `npm run test:frontend` dat 195/195; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build` va `npm run build:production` dat voi warning chunk App lon hon 500 KB da co; `git diff --check` dat. Production `docker compose --env-file .env -f compose.yaml config --quiet`, build image Chatmgt, targeted contract test 1/1 va `py_compile` trong image, `verify_tenant_isolation.py`, Chatmgt local/public health, ChatUI public `/healthz`, container health, `sudo -n nginx -t` va log scan 10 phut deu dat. `verify_deployment.py` da xac nhan database revision/credential policy nhung dung tai gate Tinode public hello vi chua co `webrtcEnabled=true`/ICE authoritative.
- Rui ro con lai: Neu Account xoa user nhung directory chi omit user ma khong tra explicit inactive/deleted, projection cua user do duoc giu cache de tranh logout nham; session cua chinh user van bi tu choi khi `/auth/me` revalidate membership. Chua UAT production bang hai tai khoan that. Full deployment verifier con phu thuoc cau hinh WebRTC/ICE cua Tinode trung tam, khong thay doi trong release nay.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, UAT bang hai tai khoan employee de xac nhan snapshot thieu khong lam mat phien va record explicit inactive van bi chan; sau do bo sung/xac nhan cau hinh WebRTC/ICE authoritative va chay lai full verifier. Khong can migration.
- Trien khai: Archive `/opt/deploy/chat/incoming/directory-sync-814fc0b-20260824-1145.tar.gz` SHA-256 `bd7e351c23423bc20202fbd45c24dcd7766b0d5ea4a488843e23ff452bfd1deb`; release `/opt/deploy/chat/releases/directory-sync-814fc0b-20260824-1145` dang la `current`, `previous` tro `/opt/deploy/chat/releases/pinned-menu-3820121-20260824-0958`; chi recreate `chatmgt` voi `--no-deps --force-recreate`, khong migration, khong `down`, khong restart ChatUI, Tinode, bridge, webhook, ChatAPI, PostgreSQL, Redis, Coturn hay volume. Image Chatmgt `sha256:9236514053bace3b2290a4e115687515bffbe9507a5aa21106864f92ab94cce8`, container `fe878a15aa42`; rollback tag `songhong-production-chatmgt:rollback-before-directory-sync-814fc0b` giu image cu `sha256:d7a8ffbace96b038b6c247568da5e202052dfe94b2b96d8502bbf895a262f8dc`; cac container ngoai `chatmgt` giu nguyen ID.
- Commit/PR: Source `814fc0b` da push `origin/master`; deployment record commit `08409c3` da tao.
...
- Database/API/cau hinh: Khong co migration, endpoint, schema, bien moi truong, secret hoac thay doi kien truc.
- Kiem thu: `npm run test:frontend` dat 195/195; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build` dat voi bundle `App-rPYYYj2t.js`; `npm run build:production` dat voi bundle `App-DJq6HDV1.js`, entry `index-DqPe-h7W.js`, CSS `index-CdssrYEE.css`. Mot lan build production chay dong thoi voi build thuong bi `ENOTEMPTY` do hai tien trinh cung don `dist/stickers`; chay lai rieng production da dat. `git diff --check` dat. Production build image dat, `chat` healthy, local/public `/healthz`, Chatmgt auth health, `sudo -n nginx -t`, public entry `index-B3oX09n_.js`, App bundle `App-BmtzpzgU.js` co marker `pinned-message-context-menu` va `fa-table-columns`, env/runtime duoc giu nguyen, container ngoai `chat` khong doi. Chua UAT pixel-level bang browser production trong phien nay.
...
- Viec tiep theo: UAT hai theme tren `https://chat.upgo.vn`; neu loi chi tro `current` ve `previous` (`group-board-a01e93c-20260824-0936`) va recreate rieng `chat`, khong restart dich vu stateful.
- Trien khai: Commit source `3820121` da push `origin/master`; archive `/opt/deploy/chat/incoming/vichat-pinned-menu-3820121.tar.gz` SHA-256 `b60715c3cf0bb1948925f65f3f829c6d26c380cec4bb526a53082effebe6d90e`; release `/opt/deploy/chat/releases/pinned-menu-3820121-20260824-0958` dang la `current`, `previous` tro `/opt/deploy/chat/releases/group-board-a01e93c-20260824-0936`; chi recreate `chat` voi `--no-deps --force-recreate`, khong migration/backup database va khong restart Chatmgt, Tinode, bridge, webhook, ChatAPI, PostgreSQL, Redis hay Coturn. Image ChatUI `sha256:569f77d325904168de8fb23462aa529755e856039e9ebb3c02265c4faf0fbf9c`, container `6ed3e145cb17fb9e8745b7f3a1a93d9e4463987c55a2ed621a9624c3b894c194`; rollback tag `songhong-production-chat:rollback-before-pinned-menu-3820121` giu image cu `sha256:2434d628ac2614b25473cd34dd8bfc1a028a259cb33912297a513723d382da2f`. Cac container ngoai `chat` giu nguyen ID.
- Commit/PR: Source `3820121` da push `origin/master`; deployment follow-up dang cho commit.
...
- Kiem thu: Local `npm run test:frontend` dat 172/172; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat; `python -m py_compile application/controllers/api_chat_management.py application/models/models.py tests/test_chat_auth_contract.py` dat; `python -m unittest discover -s tests -q` dat 183 test, skip 61 theo moi truong; `git diff --check` dat. Production build trong Docker dat voi bundle `App-Cjd4fGQH.js`, `index-77cHfEIg.js`, `index-DK7uz_Kt.css`; Alembic `20260824_12 (head)`, database/credential verifier dat, tenant-isolation verifier dat, approval contract test dat, health public/noi bo, nginx va log scan dat. Full unittest trong image con 5 loi source-only/dependency path (`/src/app/App.jsx`, `local_vichat_ai_api`) khong thuoc approval.
- Rui ro con lai: Chua UAT hai tai khoan production; HTTP verifier day du dung o gate Tinode authoritative chua quang ba `webrtcEnabled`/ICE, la cau hinh ha tang co san ngoai pham vi approval. Can kiem tra owner/member, bat tat setting, them nguoi khi pending, duyet, tu choi, F5/reconnect va subscriber Tinode.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn` va UAT approval bang owner/member. Neu loi chi tro `current` ve `previous`, recreate `chatmgt`/`chat` tu release rollback va khong reset Tinode/topic/message; backup PostgreSQL da luu truoc migration.
- Trien khai: Source `8456dcf` da push `origin/master`; archive SHA-256 `41F67BFDE4EECE73A3249571E2CC14D79C22A8058B9A34A1786361EDD07C327E`; release `/opt/deploy/chat/releases/member-approval-8456dcf-20260824-1` dang la `current`, `previous` tro `/opt/deploy/chat/releases/background-picker-2d03aa4-20260823-214315`; backup `/opt/deploy/chat/backups/chatservice-member-approval-20260824T000001Z.dump` 125634 bytes; chi recreate `chatmgt` va `chat`, khong restart Tinode, bridge, webhook, ChatAPI, PostgreSQL, Redis hay Coturn. Image ChatUI `sha256:26e134b847c7a7915948eb3508517e1f7dd0795009ba960fa008482a633e0453`, container `f6ab1e11a0a3df2f42c7c1d80b7cad4bc901ab0fca17c0e0fe6da6278034d6de`; image Chatmgt `sha256:c29f7e4c0d476e29c7d56c8e9e3aa7959739e2905cfedc22a82d2e5871422f20`, container `6c03933796d5a05954df487b51a40fa4d09a5747d465142696afca62de5fcd1a`; rollback tags `songhong-production-chat:rollback-before-member-approval-8456dcf` va `songhong-production-chatmgt:rollback-before-member-approval-8456dcf` da luu.
- Commit/PR: Source `8456dcf`; deployment follow-up `fda18e6`.
...
- Commit/PR: Source `af2440e` va changelog deploy follow-up `3873a28` da push `origin/master`; deployment da hoan tat.
...
- Commit/PR: Code `953a138`; deployment docs follow-up `2e05e0c`.
...
- Kiem thu local: `npm run test:frontend` dat 159/159; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat voi bundle `index-D7jXEG7O.js`, `App-CoGiATsj.js`, CSS `index-KipYZzQp.css`; `python -m unittest discover -s chatservice-main/tests -q` dat 181 test, skip 61 theo moi truong; `python -m py_compile chatservice-main/application/controllers/api_chat_management.py chatservice-main/tests/test_chat_auth_contract.py` dat; `git diff --check` dat. Contract test trong image production voi mount day du source/docs dat 181/181.
...
- Viec tiep theo: UAT voi hai tai khoan that: doi avatar ca nhan/nhom, hard refresh, chuyen tenant, reconnect; doi nickname o chat 1-1 va xac nhan nickname cap nhat trong group, member, reply, reaction, history va mention picker. Neu UAT gap loi, rollback `current` ve `previous` va chi recreate `chat`/`chatmgt`; khong migration/reset database, Tinode, Redis, volume hay service khac.
- Trien khai: Da deploy production tai release `/opt/deploy/chat/releases/b07e850-20260823-1933`; `current` tro release nay, `previous` tro `/opt/deploy/chat/releases/dea1dd2-20260823-1845`. Chi recreate `chat` va `chatmgt`; Tinode, bridge, webhook, ChatAPI, PostgreSQL, Redis, Coturn va volume duoc giu nguyen. Image digest: `chat` `sha256:e6d86c4500d7a5ab26f47e23f083eb02a326be6959bf581bd2b366541c1b251e`, `chatmgt` `sha256:3a35f1fb3ecf9e3aab49d1f472bcf1d5658aca7d006a49487928e636b952a73f`. Rollback tags: `songhong-production-chat:rollback-before-avatar-nickname-b07e850-20260823-1933`, `songhong-production-chatmgt:rollback-before-avatar-nickname-b07e850-20260823-1933`.
- Kiem tra production: `https://chat.upgo.vn/healthz` HTTP 200 `ok`; `https://chatmgt.upgo.vn/api/v1/auth/health` HTTP 200 `status: ok`; `chat` va `chatmgt` healthy; public entry va CSS HTTP 200, App chunk co marker `contact-nicknames`; `sudo nginx -t` dat. Co mot so request webhook 503 ngan trong luc recreate service, sau do da tu phuc hoi; log sau deploy khong thay loi fatal.
- Commit/PR: `b07e850` da commit va push `origin/master`.
...
- Kiem thu: Local `npm run test:frontend` dat 156/156; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat voi bundle `App-BJikQiI5.js` va `index-KipYZzQp.css`; `git diff --check` dat. Production release `dea1dd2-20260823-1845` co container `chat` healthy; internal/public `/healthz` HTTP 200; asset JS/CSS HTTP 200 va co marker `group-owner-avatar-badge`; `sudo -n nginx -t` dat.
- Rui ro con lai: Chua UAT pixel-level tren light/dark bang owner/member production.
- Viec tiep theo: UAT group bang tai khoan owner va member; neu can rollback, tro `current` ve `previous` va recreate rieng `chat` tu rollback tag.
- Trien khai: Da deploy production tai release `/opt/deploy/chat/releases/dea1dd2-20260823-1845`; `current` tro release nay, `previous` tro `a2efad7-20260823-1830`; chi recreate `chat`, khong dong vao `chatmgt`, Tinode, bridge, webhook, ChatAPI, PostgreSQL, Redis, Coturn hoac volume. Image moi manifest `sha256:3371ddd4122840cc29815fa5b1cbc6251429d8da7c1f0b2e9fcef508d9947784`; rollback tag `songhong-production-chat:rollback-before-owner-key-dea1dd2-20260823-1845`.
- Commit/PR: Source `dea1dd2` da push `origin/master`; follow-up docs commit.
...
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, mo thong tin group, kiem tra muc `Thanh vien nhom` thu gon/mo rong, them member trong panel va menu ba cham `Xoa khoi nhom` voi tai khoan duoc phep.
- Trien khai: Commit source `8714506` da push `origin/master`; archive SHA-256 `D3B190CE2E0F1042989295781DB2B391575B795FB2BF891ED1650ADF411D84AA`; release `/opt/deploy/chat/releases/8714506-20260823-1629`; `current` tro release moi, `previous` tro `/opt/deploy/chat/releases/443b31e-20260823-1558`; chi recreate `chat`, khong migration/reset volume/database, Chatmgt/Tinode bridge/webhook/ChatAPI/PostgreSQL/Redis/Coturn giu nguyen. Image `chat` `sha256:10c37c7c640b1cc403e6d144adf7370071cfb17095f332c584cc9287891287b5`; rollback tag `songhong-production-chat:rollback-before-group-members-8714506` giu image cu `sha256:e2f4049bcf2db8792dac5c9a81ab79d0b54f8d04d0ccacbb9fa76be4b1d0eda1`.
- Kiem tra production: Compose config dat; container `chat` healthy; local/public `/healthz` tra `ok`; public JS `App-Css79kEC.js` va CSS `index-B_xSE4Xo.css` HTTP 200 va co marker panel member; `sudo -n nginx -t` dat; log `chat` 5 phut khong co fatal/panic/traceback/critical/emerg/uncaught.
- Commit/PR: Source `8714506`; follow-up docs deployment commit.
...
- Kiem thu: Local `npm run test:frontend` dat 151/151; `python -m py_compile chatservice-main/application/controllers/api_chat_management.py chatservice-main/tests/test_chat_auth_contract.py` dat; `python -m unittest discover -s chatservice-main/tests -q` dat 178 test, skip 60 theo moi truong; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat voi bundle `App-CNmXYj-3.js`, CSS `index-DNaPZcbY.css`; `git diff --check` dat. Production image contract dat 40/40, skip 18; tenant isolation dat.
- Rui ro con lai: Chua UAT bang hai tai khoan that de xac nhan viewer A/B, hard refresh, chuyen tenant, history/reaction va profile update realtime; full `verify_deployment.py` dung o gate WebRTC/Tinode hien co (`Public Tinode hello did not confirm authoritative WebRTC/ICE configuration`), khong phat sinh tu nickname.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, UAT nickname bang hai tai khoan va xac nhan nickname khong lo sang viewer khac. Khong can migration; khong reset database, Tinode, Redis, volume hay cac service khac.
- Trien khai: Commit `7116bbe` da push `origin/master`; release `/opt/deploy/chat/releases/7116bbe-20260823-1533`; `current` tro release moi, `previous` tro `/opt/deploy/chat/releases/266bde1-20260823-1450`; chi recreate `chat` va `chatmgt`, cac container Tinode/bridge/webhook/ChatAPI/PostgreSQL/Redis/Coturn giu nguyen. Image `chat` `sha256:4cc781b29e3af957947388bf12bebe1f1a3b4c922317827b7c4e08ceb539a9d6`, image `chatmgt` `sha256:bd1f59894fa199b2f838f2637a429215415d5750d1020777fa45d7c28617d3dd`; rollback tags `songhong-production-chat:rollback-before-nickname-7116bbe` va `songhong-production-chatmgt:rollback-before-nickname-7116bbe` giu image cu.
- Kiem tra production: Hai container `healthy`; local/public Chat health va Chatmgt health tra OK; nickname endpoint chua dang nhap tra `401` (khong phai `404`); JS `App-Da6llM-t.js` va CSS `index-DNaPZcbY.css` HTTP 200 va bundle co marker nickname/API; `sudo -n nginx -t` thanh cong; log 15 phut cua chat/chatmgt/bridge/webhook khong co fatal/panic/traceback/critical/emerg/uncaught.
- Commit/PR: `7116bbe`; follow-up nay ghi nhan deploy production.
...
- Kiem thu: Local `npm run test:frontend` dat 146/146; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat voi bundle `index-D64Q6htL.js`; `python -m py_compile application/controllers/api_chat_management.py tests/test_chat_auth_contract.py` dat; `python -m unittest discover -s tests -q` dat 172 test, skip 56 theo moi truong; Pillow xac nhan 112/112 PNG RGBA; `git diff --check` dat. Server archive SHA-256 `8c03af5454b643f7521cac205dc603fdd1b6011fddd0da7637a69b56de1dd3da`; server build `chat`/`chatmgt` dat; contract trong image dat 38/38 voi 16 source-only skip; `nginx -t` dat; database verifier va two-tenant isolation dat; `chat` va `chatmgt` healthy; public `/healthz`, Chatmgt health, bundle `/assets/index-BF7Zg2Ey.js`, CSS `/assets/index-BKcXFzbk.css` va sticker HTTP 200; toan bo container production van Up/healthy. Full `verify_deployment.py` dung o gate WebRTC public vi authoritative Tinode khong tra `webrtcEnabled/ICE` theo verifier hien co; khong lien quan thay doi release nay.
- Rui ro con lai: Can UAT bang owner va member that tren hai phien dang nhap de xac nhan avatar group cap nhat realtime, sticker gui/nhan va viewer anh; gate WebRTC public van can xu ly rieng.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, test 4 pack sticker, tao group co/khong co member khac, viewer zoom/share/download va doi avatar group tren hai tai khoan. Neu can rollback, tro `current` ve `previous` va dung image rollback da tag; khong xoa volume/database.
- Trien khai: Commit `4211866` da push `origin/master`; release `/opt/deploy/chat/releases/4211866`; `current` tro release moi, `previous` tro `/opt/deploy/chat/releases/group-controls-0b2f31e-20260823-0530`; image ChatUI `sha256:06b3eb935f51310e0aa14e568c63a509fffb0fb863dca5fe28cc97043c7dc2b4`, image Chatmgt `sha256:7c1cba8d04298a334867a94ccb478f2d0b1c0cd3c929433591f6c7d7016df98e`; rollback tag giu ChatUI `sha256:ebf594742d1f290770e504729d0186422870cbe8d55fccadeda8b98dbf6fcb97` va Chatmgt `sha256:c78c8e960957e2e3b8496e39121eb5e29740acf2c346a54ad5e73d5620663d59`; chi recreate `chatmgt` va `chat`, khong migration/reset volume/database, Tinode/bridge/webhook/ChatAPI/PostgreSQL/Redis/Coturn giu nguyen.
- Commit/PR: `4211866`; docs deployment follow-up dang cho commit.
...
- Kiem thu: Local `npm run test:frontend` dat 146/146; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat; `python -m py_compile application/controllers/api_chat_management.py tests/test_chat_auth_contract.py` dat; `python -m unittest tests.test_chat_auth_contract -q` dat 37/37; `git diff --check` dat. Production compose config dat; Chatmgt image contract dat 37 test, skip 15 source-only; `chat` va `chatmgt` healthy; public `http://127.0.0.1:8094/healthz`, Chatmgt health tren bind noi bo, `https://chat.upgo.vn/healthz` deu tra ok; public assets `index-phEoq3Ug.js`, `App-C5ImAi9b.js`, `index-L1ZBq6u_.css` HTTP 200 va bundle co marker `reaction-details-modal`, `notification-mute-duration`, `fa-key`; container Nginx va host `sudo nginx -t` dat; log ChatUI, Chatmgt, Tinode bridge va chatbot webhook trong 5 phut khong co fatal/panic/traceback/uncaught/critical/emerg.
- Rui ro con lai: Chua UAT bang tai khoan owner va member tren group cu/moi, reaction tren tin nhan text/file/sticker, mute tren desktop/mobile; can hard refresh de nhan bundle moi.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, test bang owner va member: mo dialog mute, them thanh vien tu member thuong, xem chia khoa vang tren tin nhan admin group, bam pill reaction de xem nguoi; neu loi thi tro `current` ve `previous` va recreate lai chi `chat`/`chatmgt` tu release cu.
- Trien khai: Source `0b2f31e` da push `origin/master`; archive SHA-256 `C4073E6FAA3D36E44AF38E9702F1AA881C6262837CAA368FE555BCC2C23C3CCC`; release `/opt/deploy/chat/releases/group-controls-0b2f31e-20260823-0530`; image ChatUI `sha256:ebf594742d1f290770e504729d0186422870cbe8d55fccadeda8b98dbf6fcb97`, container `2794aed5ff36e5d682d83104ccc3c77dcb4b7258ff71d78cdc93f66ed4615e79`; image Chatmgt `sha256:c78c8e960957e2e3b8496e39121eb5e29740acf2c346a54ad5e73d5620663d59`, container `a5c0a87de18e5404df8e25ebf80d79a8643f8ec7921eeb5318b6d7e4216f89c6`; rollback tags `songhong-production-chat:rollback-before-group-controls-0b2f31e-20260823-0530` (image cu `sha256:b7494c7696b8822ff702c1626dba7f4aad26b9c7b131044a50a9bce14bb8afff`) va `songhong-production-chatmgt:rollback-before-group-controls-0b2f31e-20260823-0530` (image cu `sha256:b0c5c6e1c7a4f3319214978b1025fd3dc1b375ce86625698acc0708ef10d891d`); `current` tro release moi, `previous` tro `/opt/deploy/chat/releases/group-info-bf74edd-20260823-043033`; chi recreate `chatmgt` va `chat`, khong migration, khong `down`, khong reset volume/database, Tinode/bridge/worker/ChatAPI/PostgreSQL/Redis/Coturn giu nguyen.
- Commit/PR: Source `0b2f31e`; deployment verification `d2e256a`; khong co PR.
...
- Database/API/cau hinh: Khong co migration, endpoint, bien moi truong, secret hoac thay doi quyen.
- Kiem thu: `npm run test:frontend` dat 142/142; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat; `git diff --check` dat.
- Rui ro con lai: Chua UAT click dialog bang tai khoan production co nhieu membership; khong co loi health/log da biet.
...
- Rui ro con lai: Chua UAT bang tai khoan production de xac nhan lich su cu, mention bot trong group, style cau tra loi va membership reconcile; chua chay browser pixel/UAT end-to-end trong session nay. Rollback source duoc giu tai release `pinned-theme-30eb900-20260819-1305`.
...
- Loai: Tinh nang | Web | UI | Kiem thu
- Trang thai: Da deploy production; san sang UAT
- Muc tieu: Rut gon thao tac voi tin nhan theo kieu Zalo, tu dong nhac den nguoi duoc tra loi va truy xuat nhanh tin nhan da ghim.
- Pham vi: Thanh thao tac tin nhan trong ChatUI, composer mention, reaction picker, menu ba cham, ghim tin nhan va test frontend; khong thay doi Tinode message/reaction contract, Chatmgt, database hay cac luong realtime.
- File da thay doi: `src/app/App.jsx`, `src/styles/index.css`, `src/features/chat/services/messageActionPolicy.js`, `src/features/chat/services/messageActionPolicy.test.js`, `src/features/i18n/appLanguage.js`, `src/features/i18n/appLanguage.test.js`, `package.json`, `dist/index.html`.
- Noi dung: Them icon Reply/Share/Like o ngoai bubble; hover Like mo bo chon bieu cam; Reply trong group tu chen mention nguoi gui va luu mention metadata; menu ba cham them Ghim/Bỏ ghim va bo cac muc da dua ra quick action; header hien preview cac tin da ghim, bam vao se cuon den tin nhan va highlight tam thoi.
- Quyet dinh ky thuat: Dung handler message action hien co de giu nguyen Tinode reaction/share/reply; pin tin nhan dung `message-actions` theo viewer/device vi backend Tinode hien chua co API pin message, khong tao event rieng lam nhiễm tin realtime.
- Database/API/cau hinh: Khong co migration, endpoint, secret hoac bien moi truong; pin message chi dong bo trong localStorage cua tai khoan tren thiet bi hien tai.
- Kiem thu: `npm run test:frontend` dat 131/131; `npm run lint` exit 0 voi warning legacy tai `src/App.jsx` va vendor `public/ChatBotWidget/tinode.js`; `npm run build:production` dat; `git diff --check` dat. Production `docker compose config -q`, build service `chat`, local `/healthz`, public `https://chat.upgo.vn/healthz`, public bundle HTTP 200 co marker `message-quick-actions`, `pinned-messages-strip`, `message-reaction-picker`, `sudo nginx -t`, Chatmgt auth health va log ChatUI 5 phut khong co fatal marker deu dat.
...
- Trang thai: Hoan tat; da push va deploy production; san sang UAT
...
- Kiem thu: `npm run test:frontend` dat 124/124; `npm run lint` exit 0 voi warning legacy da co; `npm run build:production` dat; `git diff --check` dat. Production `docker compose config -q` va build service `chat` dat; local/public `/healthz` tra `ok`; Chatmgt auth health tra `status=ok`; bundle public `index-DT_gJeg6.js` va `App-DcJaDH-i.js` HTTP 200, co marker `directProvisioning` va `pendingDirect`; log ChatUI 5 phut khong co `panic/fatal/traceback/critical/emerg/uncaught`.
- Trien khai: Commit source `1156017` da push `origin/master`; release `/opt/deploy/chat/releases/direct-navigation-1156017-20260818-1915`; archive SHA-256 `CCAE8371E763F489E6C67C4CD28E62E8061694AA1A8FCD49B84414BFFDE4DFB7`; image ChatUI `sha256:4ad1741f74a621b16ce914ba16e901ea288db8ca3cf46b4b0b4f7f2d13b817b3`; container moi `0780e91c6227`; `current` tro release moi, `previous` tro `conversation-state-784b9e1-20260818-102838`; chi recreate `chat`, giu nguyen Chatmgt/Tinode bridge/chatbot, ChatAPI, PostgreSQL, Redis, Coturn va volume.
- Rui ro con lai: Browser runtime cua agent khong co phien dang nhap de UAT truc quan; can hard refresh production va bam `Danh ba -> Nhan tin` bang tai khoan employee de xac nhan UX thuc te. Rollback filesystem san sang qua `previous`; image cu dang chay truoc deploy khong con layer local de gan tag moi sau khi build, nen rollback image exact can build lai tu release previous neu can.
- Viec tiep theo: UAT hard refresh voi tai khoan employee; kiem tra room cu, room moi, gui tin, reload va nhom.
- Commit/PR: Source `1156017`; docs follow-up dang cho commit
...
- Kiem thu: `npm run test:frontend` dat 123/123; `npm run lint` exit 0 voi warning legacy da co tai `src/App.jsx` va `public/ChatBotWidget/tinode.js`; `npm run build:production` dat; `git diff --check` dat. Remote archive SHA-256 `020BD30D5E47C78481FAF1AAF603DCE2621BAABF230C0E832F5EBC1C0E03C73C`; `docker compose config -q` va build service `chat` dat; ChatUI local/public health tra `ok`; public entry `index-XiwAbU6s.js` va bundle `App-Dejy3pbb.js` co marker `TINODE_MEMBERSHIP_FAILED`; log ChatUI 5 phut khong co traceback/panic/fatal/critical/emerg/exception.
- Trien khai: Release `/opt/deploy/chat/releases/conversation-render-2c23df6-20260818-095516`; `current` tro release moi, `previous` tro `managed-retry-232889d-20260818-153520`; image ChatUI `sha256:c19a4ecba54f6d37eda73680cd7a39bf2665bd0e477e4047a4c293c1eabcdea4`, container moi `e874a1d3dd3e` (cu `28638f2aabc8`). Rollback tag `songhong-production-chat:rollback-before-conversation-render-2c23df6`; cac container ngoai `chat` khong doi ID.
- Rui ro con lai: Chua hard refresh/UAT voi tai khoan UpGO that tren production, chua xac nhan payload danh ba va room 1-1 qua server Tinode trong trinh duyet.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, test danh ba -> Nhan tin, mo room cu, tao room moi, gui tin, nhom, them/xoa thanh vien va reload bang it nhat hai tai khoan cung tenant.
- Commit/PR: Source `2c23df6`; docs follow-up dang cho commit.
...
- File da thay doi: `src/app/App.jsx`, `src/features/chat/services/chatRealtime.js`, `src/features/chat/services/chatRealtime.test.js`, `src/features/chat/services/chatManagementService.js`, `src/features/chat/services/chatManagementService.test.js`, `chatservice-main/application/services/auth_service.py`, `chatservice-main/application/controllers/api_chat_management.py`, `chatservice-main/tests/test_tinode_bridge_service.py`, `chatservice-main/tests/test_chat_auth_contract.py`, `docs/CHANGELOG.md`; release `/opt/deploy/chat/releases/managed-retry-232889d-20260818-153520`.
...
- Kiem thu: `npm run test:frontend` dat 122/122; `npm run lint` exit 0 voi warning legacy tai `src/App.jsx` va `public/ChatBotWidget/tinode.js`; `npm run build:production` dat, bundle local tao `App-BsmhbJfQ.js`; `python -m unittest discover -s tests -v` lan chay xac nhan dat 167 test, bo qua 56 test do dependency chi co trong image Chatmgt (lan dau gap mot loi localhost Windows `WinError 10053`, test do va toan suite deu dat khi chay lai); `python -m unittest tests.test_tinode_bridge_service -v` dat 18/18 voi `aiohttp 3.10.11` va `bcrypt 4.2.1` trong thu muc tam; `python -m unittest tests.test_chat_auth_contract -v` dat 36/36; `python -m py_compile application/services/auth_service.py application/controllers/api_chat_management.py tests/test_tinode_bridge_service.py tests/test_chat_auth_contract.py` va `git diff --check` dat. Archive production SHA-256 `C714573AB80C4439C0ABD334C7F59EDD07B42DC6CF505A7DEDBE32A34DDD8CF0`; Compose config dat; bridge test trong image Chatmgt dat 18/18; Nginx config dat tren network production; Chatmgt/ChatUI healthy; public ChatUI va Chatmgt health HTTP 200; bundle public `App-ChSmsaxb.js` HTTP 200 co marker `TINODE_TOKEN_REQUIRED`, `TINODE_MEMBERSHIP_FAILED` va `managementSnapshot`; log sau deploy khong co traceback/panic/fatal/critical/emerg.
- Trien khai: `current` tro release moi, `previous` tro `managed-chat-d5d2594-20260818-071610`; image/container Chatmgt `sha256:b7e49b0d3d93`/`3c4e8a069384`, ChatUI `sha256:6b8e25f229b9`/`28638f2aabc8`; rollback tag da gan cho hai image cu. Chi recreate `chatmgt` va `chat`; ChatAPI, hai PostgreSQL, Redis, Tinode account bridge, chatbot webhook va Coturn giu nguyen container ID.
- Rui ro con lai: Chua co browser session/tai khoan production de UAT thao tac that; can xac nhan room 1-1, them/xoa thanh vien nhom va gui tin sau hard refresh.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, UAT bang tai khoan that; neu van loi, ghi lai thao tac, thong bao tren man hinh va thoi diem de doi chieu request/log production.
- Commit/PR: Source `232889d`; docs `5821faf`; deploy follow-up dang cho commit.
...
- Database/API/cau hinh: Khong co migration, endpoint, secret hoac bien moi truong moi.
- Kiem thu: `npm run test:frontend` dat 115/115; `npm run lint` exit 0 voi warning legacy tai `src/App.jsx` va vendor `public/ChatBotWidget/tinode.js`; `npm run build:production` dat; `git diff --check` dat. Remote archive SHA-256 `D12E99C474B197B19517401FC6D5B6082170DB1BFDC7C9A3FE12433097EA8A60`; Compose `config -q` va build ChatUI dat; image `sha256:04b0362010544cd52151dc82a4594bb331ad0e0c86c311651d0baaa6f83336c2`; container `1fc0f745546917054b13522066d2ccba44ce51f1846d206ec092acda2c75a9c3` healthy; local/public `/healthz` tra `ok`; bundle `index-U3k_J-T7.js` va `App-L4AWzBWe.js` HTTP 200; Chatmgt auth health tra `status=ok`; `sudo nginx -t` dat; log ChatUI sau recreate khong co fatal/exception marker; khong recreate Chatmgt, Tinode bridge/webhook, ChatAPI, PostgreSQL, Redis hoac Coturn.
...
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, mo room 1-1 dang loi va mot room moi de xac nhan sidebar, ten, tin nhan, realtime va khong con man hinh trang.
- Commit/PR: Source `a234415`; docs follow-up dang cho commit; release `/opt/deploy/chat/releases/direct-render-a234415-20260818-133248`.

## 2026-08-18-14 - Bao ve render khi du lieu chat 1-1 khong day du

- Thoi gian: 2026-08-18 12:50 (Asia/Saigon)
- Loai: Sua loi | Web | Van hanh | Kiem thu
- Trang thai: Da deploy production; san sang UAT
...
- File da thay doi: `src/features/chat/services/chatRealtime.js`, `src/features/chat/services/tinodeClient.js`, `src/features/chat/services/chatManagementService.js`, `src/features/contacts/services/accountDirectory.js`, `src/app/App.jsx`, `src/RootApp.jsx`, cac file test lien quan; release `/opt/deploy/chat/releases/malformed-direct-f76b974-20260818-1240`.
...
- Database/API/cau hinh: Khong co.
- Kiem thu: Local `npm run test:frontend` dat 114/114; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat; `git diff --check` dat. Remote archive SHA-256 `37E3D66557AD8402AC7E28784295BAD75E415BC5B1F01676C47D0406681FC404`; Compose config dat; image ChatUI `sha256:3636fa95b8a66360133204c4ba46bc0bddf5a3eb26c68457135d33ad1e7d0b0d`; container `4b0615d82736` healthy; local/public `/healthz` tra `ok`; public entry `index-CLWTX2st.js` va App bundle `App-CDXW7gnD.js` HTTP 200; Chatmgt auth health tra `status=ok`; log ChatUI sau recreate khong co fatal/exception marker; khong recreate Chatmgt, Tinode bridge/webhook, ChatAPI, PostgreSQL, Redis hoac Coturn.
- Rui ro con lai: Chua UAT truc quan bang tai khoan UpGO/Tinode that cho room 1-1; browser runtime cua agent khong tao duoc kernel asset. Can hard refresh de tai entry hash moi neu tab dang giu bundle cu.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, mo chat 1-1 dang loi va mot room moi de xac nhan ten, tin nhan, realtime va sidebar; rollback neu phat sinh loi moi.
- Commit/PR: Source `f76b974`; docs `7e67324`; release da deploy.
...
- File da thay doi: `src/app/App.jsx`, `src/features/chat/services/chatRealtime.js`, `src/features/chat/services/chatRealtime.test.js`, `docs/CHANGELOG.md`; release `/opt/deploy/chat/releases/direct-room-6b3315f-20260818-115228`.
- Noi dung: Them ham lay ten phong an toan cho chuoi, so va fallback theo thanh vien; dung fallback khi merge room realtime, tao room 1-1 va render sidebar; bo loi goi `.toLowerCase()` tren ten phong co the bi thieu. Source archive SHA-256 `78BEEB3794468101FCA629F62DEAADFE59B137554EB468FF1AF6ED42D75EBC34`.
...
- Database/API/cau hinh: Khong co.
- Kiem thu: Local `npm run test:frontend` dat 111/111; `npm run lint` exit 0 voi warning legacy/vendor da co; `npm run build:production` dat; `git diff --check` dat. Remote Compose `config -q` dat; image ChatUI `sha256:83690c9b6d965c9a984e7d12acc9f3210b4c63fca87addac1f6484184ee70bd7` va container `85f09912d8a` healthy; `/healthz` noi bo/public tra `ok`; public App bundle `App-CYMCvXtO.js` HTTP 200; Chatmgt auth health tra `status=ok`; log ChatUI 10 phut khong co fatal/exception marker; khong recreate Chatmgt, Tinode bridge/webhook, ChatAPI, PostgreSQL, Redis hoac Coturn.
- Rui ro con lai: Chua UAT truc quan bang tai khoan UpGO/Tinode that tren chat 1-1; Docker khong con content layer de gan tag image rollback cu, nhung release `previous` va source production van duoc giu de rebuild khi can.
- Viec tiep theo: Hard refresh `https://chat.upgo.vn`, mo mot chat 1-1 co tin nhan va mot room moi de xac nhan ten, tin nhan, realtime va sidebar.
- Commit/PR: Source `6b3315f`; release da deploy; docs follow-up dang cho commit.
...
- Noi dung: Account tra danh sach membership active an toan; ChatUI hien icon cong ty canh Dang xuat trong Ho so khi co nhieu cong ty; `POST /api/v1/auth/switch-tenant` kiem tra membership, goi Account `POST /api/v1/tenant/set_current_tenant`, bat `Set-Cookie` neu Account xoay cookie, doc lai `/current_user`, tao projection/Tinode mapping theo tenant moi va chi xoay JWT Chatmgt. Frontend xoa state Tinode/danh ba cu va reload de tai luong moi.
...
- Commit/PR: Hotfix `e4c7155`; docs follow-up `06ae56f` da push `origin/master`.
